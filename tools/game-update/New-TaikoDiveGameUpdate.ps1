...
if (-not (Test-Path -LiteralPath (Join-Path $buildRoot 'TaikoDive.exe') -PathType Leaf)) {
    throw "TaikoDive.exeがありません: $buildRoot"
}
...
if ([string]::IsNullOrWhiteSpace($SevenZipPath)) {
    ...
    if ($null -ne $sevenZipCommand) {
        ...
        foreach ($programFilesRoot in @($env:ProgramW6432, $env:ProgramFiles)) {
            ...
            $defaultSevenZip = Join-Path $programFilesRoot '7-Zip\7z.exe'
...
$packageName = "TaikoDive_Update_v${Version}_win-x64_$($revisionLower.Substring(0, 7)).zip"
$outputRoot = [System.IO.Path]::GetFullPath($OutputDirectory)
[System.IO.Directory]::CreateDirectory($outputRoot) | Out-Null
$packagePath = Join-Path $outputRoot $packageName
$manifestPath = Join-Path $outputRoot 'game-update-manifest.json'
if (Test-Path -LiteralPath $packagePath) {
    throw "出力先に同名パッケージがすでにあります: $packagePath"
}

$temporaryRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("TaikoDiveGameUpdate-" + [Guid]::NewGuid().ToString('N'))
...
try {
    ...
    foreach ($sourceFile in Get-ChildItem -LiteralPath $buildRoot -File -Recurse) {
        if (($sourceFile.Attributes -band [System.IO.FileAttributes]::ReparsePoint) -ne 0) {
            throw "再解析ポイントはパッケージに含められません: $($sourceFile.FullName)"
        }

        $relativePath = [System.IO.Path]::GetRelativePath($buildRoot, $sourceFile.FullName).Replace('\', '/')
        ...
        if ($relativePath -ieq 'Log.txt' -or $relativePath.EndsWith('.pdb', [StringComparison]::OrdinalIgnoreCase)) {
            continue
        }
    ...
    $internalManifestJson = $internalManifest | ConvertTo-Json -Depth 5
    [System.IO.File]::WriteAllText(
        (Join-Path $payloadRoot 'package-files.json'),
        $internalManifestJson + [Environment]::NewLine,
    ...
    $externalManifestJson = $externalManifest | ConvertTo-Json -Depth 5
    [System.IO.File]::WriteAllText(
        $manifestPath,
        $externalManifestJson + [Environment]::NewLine,
        [System.Text.UTF8Encoding]::new($false))
